...
export const listarTarefasResolve:
    ResolveFn<ListarTarefasViewModel[]> = () => {
        return inject(TarefasService).selecionarTodos(StatusTarefa.Todos)
    };

export const buscarTarefaPorIdResolve:
    ResolveFn<[FormTarefasViewModel]> = (route: ActivatedRouteSnapshot) => {
        return inject(TarefasService).selecionarPorId(route.params['id'])
    }

export const buscarTarefaCompletaPorIdResolve:
    ResolveFn<[VisualizarTarefasViewModel]> = (route: ActivatedRouteSnapshot) => {
