...
export const formCompromissoResolve:
    ResolveFn<[FormCompromissoViewModel]> = (route: ActivatedRouteSnapshot) => {
        return inject(CompromissoService).selecionarPorId(route.paramMap.get('id')!)
    };

export const listarCompromissoResolve:
    ResolveFn<ListaCompromissosViewModel[]> = () => {
        return inject(CompromissoService).selecionarTodos()
    };

export const compromissoCompletoResolve:
